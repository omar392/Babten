...
class CoversController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->file('image_jobs')) {
            $file = $request->file('image_jobs');
            $filename = date('YmdHi') . $file->hashName();
            $destinationPath = public_path('/dashboard');
            $file->move($destinationPath, $filename);
            $cover['image_jobs'] = $filename;
        }
        if ($request->file('image_jobs_2')) {
            $file = $request->file('image_jobs_2');
            $filename = date('YmdHi') . $file->hashName();
            $destinationPath = public_path('/dashboard');
            $file->move($destinationPath, $filename);
            $cover['image_jobs_2'] = $filename;
        }
